...
class Square(tk.Button): #the squares in this Game of Life will be buttons, so that the user can select which one to start as alive
    ...
    def add_neighbours(self, cell_list): #method that adds the cells neighbours to a list. intended to be called once at the beginning of the game.
        neighbours = []
    ...
    def check(self, cell_list): #checks the neighbours and counts how many are alive
        alive = 0
    ...
    def update(self, cell_list): #updates the cells status based on how many neighbours are alive
...
class Board(): #Class containing the board, a list of all cells, and the methods to update all cells. 
    def __init__(self):
        ...
        self.side_confirm_button = tk.Button(self.side_menu, text = "create board/change size", command = lambda: self.create_board(self.side.get()))
        self.side_confirm_button.pack()
        self.advance_button = tk.Button(self.side_menu, text = "Advance state", command = lambda: self.refresh())
        self.advance_button.pack()
        self.play_button = tk.Button(self.side_menu, text = "Play/Pause", command = lambda: self.play_toggle())
        self.play_button.pack()
        self.clear_button = tk.Button(self.side_menu, text = "Clear", command = lambda: self.clear())
        self.clear_button.pack()
        self.save_button = tk.Button(self.side_menu, text = "Save state", command = lambda: self.save_state(tk.filedialog.asksaveasfilename(initialdir = os.getcwd(), title = "Save state of board", filetypes=(("text files", ".txt"),))))
        self.save_button.pack()
        self.load_button = tk.Button(self.side_menu, text = "Load state", command = lambda: self.load_state(tk.filedialog.askopenfilename(initialdir = os.getcwd(), title = "Load a saved state", filetypes=(("text files", ".txt"),))))
        self.load_button.pack()

        self.shift_field = tk.Frame(self.side_menu)
        self.shift_field.pack()
        self.shift_up_button = tk.Button(self.shift_field, text = "up", command = lambda: self.shift_state("up"))
        self.shift_up_button.grid(row = 0, column = 1)
        self.shift_down_button = tk.Button(self.shift_field, text = "down", command = lambda: self.shift_state("down"))
        self.shift_down_button.grid(row = 2, column = 1)
        self.shift_right_button = tk.Button(self.shift_field, text = "right", command = lambda: self.shift_state("right"))
        self.shift_right_button.grid(row = 1, column = 2)
        self.shift_left_button = tk.Button(self.shift_field, text = "left", command = lambda: self.shift_state("left"))
        self.shift_left_button.grid(row = 1, column = 0)
    
    def cell(self, row, col): #returns a specific cell instance based on the (row, col) arguments [int]
        for i in self.cell_list:
            if i.row == row and i.col == col:
                return i
    
    def refresh(self): #advances the board one step, by calling the relevant methods in the cells. Must be run as separate loops.
        alive = 0
        for i in self.cell_list:
            i.check(self.cell_list)
        for i in self.cell_list:
            i.update(self.cell_list)
            if i.status == "alive":
                alive += 1
    ...
    def save_state(self, filename): #saves the state of all cells in the board to a .txt file. 
        if self.playing == True:
            self.play_toggle()
        if filename == "":
            return
    ...
    def load_state(self, filename): #loads a previously saved state from a .txt file. Promts to create a larger board if the save is a larger board than the current.
        if self.playing == True:
            self.play_toggle()
        if filename == "":
            return
        ...
        if len(loaded) > len(self.board_list):
            size_answer = popup.askyesnocancel(message = "The saved state is larger than the current board, \nwould you like to create a board of the same size({})?".format(len(loaded)))
            if size_answer == True:
        ...
        for i in self.cell_list:
            if i.status == "alive":
                clear_answer = popup.askyesnocancel(message = "Clear the current board before loading?")
    ...
    def shift_state(self, dir): #shifts the state of the board by one step in the direction "str". Only one direction at a time.
        if self.playing == True:
            self.play_toggle()
        oldstate = []
        for row in self.board_list:
        ...
        for row_n, row in enumerate(oldstate):
            ...
            for col_n, col in enumerate(row):
                if col_n + col_shift not in range(len(self.board_list[row_n])):
                    continue
                self.board_list[row_n + row_shift][col_n + col_shift].setstatus(col)
        
    def create_board(self, side): #creates a square board with side length given in the entry field, checks if it's an int, removes the old board if there is one.
        if self.playing == True:
            self.play_toggle()
        try:
            side = int(side)
...
def save_to_file(filename, data): #writes data to a file. If data is a list, it gives each element a new line. 
...
def read_from_file(filename): #reads data from a file, returns a list of lines, each line is .strip()ped. 
